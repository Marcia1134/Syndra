...
class RPGroup(app_commands.Group):
    ...
    async def remove(self, interaction : Interaction, role : str) -> None:
        ...
        await interaction.response.send_message(f"Role: {role} has been removed", ephemeral=True)
    ...
    async def list(self, interaction : Interaction) -> None:
        ...
        await interaction.response.send_message(rolepay_list, ephemeral=True)
    ...
    async def pay(self, interaction : Interaction) -> None:
        ...
        rolepay = tables.RolePay.select().where(tables.RolePay.server == interaction.guild_id)
        if not rolepay:
            await interaction.response.send_message("No role pay entries found", ephemeral=True)
            return
        ...
        # Send a response message with the role pay details
        await interaction.response.send_message("Paying all role pay entries", ephemeral=True)
